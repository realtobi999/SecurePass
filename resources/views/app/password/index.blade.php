    ...
    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 relative">
        ...
        <td class="px-3 py-2">
            <input type="password" value="{{ $password->password }}" readonly
                class="border-none rounded-lg cursor-pointer"
        ...
        <td class="px-6 py-4 relative">
            <div class="cursor-pointer" onclick="toggleDropdown(event, {{ $password->id }})">
                <ion-icon name="ellipsis-vertical-outline"></ion-icon>
            </div>
            <!-- Dropdown panel, hidden by default -->
            <div class="hidden fixed mt-2 w-30 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 z-10 "
                id="dropdownMenu{{ $password->id }}">
                <div class="py-1" role="menu" aria-orientation="vertical">
                    <div class="text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100 cursor-pointer"
                        role="menuitem" onclick="showEditPassword()" tabindex="-1">
                        Edit
                    </div>
